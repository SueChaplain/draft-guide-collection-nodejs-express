...
**Projects (Local)**. The **Template Source Manager** view opens, which is shown in the following screenshot:
...
Click **Refresh** to ensure that you are viewing all the available template sources. To enable Kabanero Collections as a
template source, click on the switch. To minimize the list of available templates when you start your project, click on the
switch for any other sources that are enabled to deselect them. Codewind is now ready to use the *public* Kabanero Collection Hub.

To point to an enterprise-specific Kabanero Collection Hub URL, or a local clone, click the **Add New** button and enter the URL to
the index file in the pop-up field.
